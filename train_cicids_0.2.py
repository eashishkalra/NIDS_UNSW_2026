...
df[label_col] = df[label_col].apply(group_attacks)
...
joblib.dump(feature_names,'cicids_feature_names.pkl')
...
try:
    ...
    target_minority = int(majority_count * 0.2)  # Over-sample minorities to 10% of majority
    
    print(f"\nHybrid resampling strategy:")
    print(f"  Majority class: {le.classes_[majority_class]} ({majority_count:,} samples)")
    print(f"  Target for minorities: {target_minority:,} samples (20% of majority)")
...
class NodeClassificator(nn.Module):
    def __init__(self, dataset, num_classes, num_convs: int = 8, hid: int=128, dropout: float = 0.5, 
        ...
        self.lin1 = nn.Linear(dataset, hid)
        ...
        self.fc1 = nn.Linear(bigru_hidden_size*2,hid)
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        batch_size = x.size(0)
        usable = (batch_size // self.timestamp)*self.timestamp
        if usable != batch_size:
            x = x[:usable]
        if usable == 0:
            return x.new_zeros((0,self.fc2.out_features)), 0
        new_batch_size = usable // self.timestamp
        x = x.view(usable//self.timestamp, self.timestamp,-1)
        x,_ = self.bigru(x)
        x = x[:,-1,:]
        x=F.gelu(self.fc1(x))
        x=self.dropout(x)
        x=self.fc2(x)
        return x,new_batch_size
...
try:
    model = NodeClassificator(
            ...
            theta=0.7,
            dropout=0.5,
            bigru_hidden_size=64, bigru_layers=2, timestamp=5
        ).to(DEVICE)
except Exception as e:
    print("cant initalise model")
    print(e)
    sys.exit(0)
print("using gpu")
...
best_acc = 0.0
patience = 15  # Increased patience for 50 epoch training
patience_counter = 0
best_epoch = 0
...
for epoch in range(1, EPOCHS+1):
    ...
    for Xb, yb in train_loader:
        try:
            ...
            edge_index = create_fully_connected_edge_index(batch_size).to(DEVICE)
            edge_attr = None  # No edge attributes in this case, but could be added if needed
            if total_loss == 0 and correct == 0:
                try:
                    ...
                    pass
            optimizer.zero_grad()
            out, new_batch_size = model(Xb, edge_index, edge_attr)
            if new_batch_size == 0:
                continue
            if new_batch_size != yb.size(0):
                usable = new_batch_size * model.timestamp
                yb_adjusted = yb[:usable]
                yb_adjusted = yb_adjusted.view(new_batch_size, model.timestamp)[:, -1]  # Take the last label in each sequence
            else:
                yb_adjusted = yb

            loss = criterion(out, yb_adjusted)
            loss.backward()
            optimizer.step()
            total_loss += loss.item()
            preds = out.argmax(dim=1)
            correct += (preds == yb_adjusted).sum().item()
        except Exception as e:
    ...
    with torch.no_grad():
        for Xb, yb in test_loader:
            ...
            y_true_epoch.extend(yb_adjusted.cpu().numpy().tolist())
